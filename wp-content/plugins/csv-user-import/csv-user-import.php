...
function csvuserimport_mailer_updater($user_refid = null,$user_id=null,$user_email = null,$user_pass = null )
{
	if(empty($user_refid))
		return;
	...
	if(!empty($user_email) && !empty($user_pass))
	{
		$message = __( 'Hi,
				...
				Please login to the site by clicking %5$s and following the login steps' );

		wp_mail( $user_email, sprintf( __( '[%s] Registration' ), get_option( 'blogname' ) ), sprintf($message, get_option('blogname'), home_url(), $user_email ,$user_pass , home_url()));
	}
	...
	if ($_POST['mode'] == "submit") {
		...
		if (is_array($arr_rows)) {
			...
			foreach ($arr_rows as $row) {
				...
				$user_email 	= trim($arr_values[0]);
				$user_refid		= trim($arr_values[1]);
						
				$user_pass		= wp_generate_password();
						
				if (!$user_email) { $username."@donotreply.com"; }

				$user_data = array('user_email' => $user_email,'user_login' => $user_email,'user_pass' => $user_pass,'role' => 'subscriber');
				...
				{
					csvuserimport_mailer_updater($user_refid,$user_id,$user_email,$user_pass);
				}
...
<div class="wrap">	
	...
	<p><a href="<?php echo plugin_dir_url(__FILE__).'test_csv.csv';?>" target="_blank">View Example</a></p>

	<p><strong>Note:</strong></p>
	
	<ol>
		<li>Please make sure that each email id and refid entered in the CSV is on a single line.</li>
