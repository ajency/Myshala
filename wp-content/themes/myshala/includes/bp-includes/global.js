...
jq(document).ready( function() {
	...
	jq('div.activity').live('click', function(event) {
		...
		if ( target.parent().hasClass('load-more') ) {
			jq("#content_1 li.load-more").addClass('loading');
			...
			function(response)
			{
				jq("#content_1 li.load-more").removeClass('loading');
				jq.cookie( 'bp-activity-oldestpage', oldest_page, {
					path: '/'
				} );
				jq("#content_1 ul.activity-list").append(response.contents);
	...
	jq("select#message-type-select").change(
		function() {
			...
			var checkboxes = jq("div.thread-options input[type='checkbox']");
			...
			switch(selection) {
				case 'unread':
					var checkboxes = jq("div.unread div.thread-options input[type='checkbox']");
					break;
				case 'read':
					var checkboxes = jq("div.read div.thread-options input[type='checkbox']");
					break;
			}
			if ( selection != '' ) {
				checkboxes.each( function(i) {
	...
	jq("a#delete_inbox_messages, a#delete_sentbox_messages").live('click', function() {
		checkboxes_tosend = '';
		checkboxes = jq("div#message-threads div.m-container div.thread-options input[type='checkbox']");
		...
		jq.post( ajaxurl, {
			...
			if ( response[0] + response[1] == "-1" ) {
				jq('div#message-threads').prepend( response.substr( 2, response.length ) );
			} else {
				jq('div#message-threads').before( '<div id="message" class="updated"><p>' + response + '</p></div>' );
...
function bp_filter_request( object, filter, scope, target, search_terms, page, extras ) {
	...
	function(response)
	{
		...
		//Members loop custom actions
		jq('li.directory-member-types').prevAll().removeClass('active');
		jq('li.directory-member-types').nextAll().removeClass('active');
		jq("li.directory-member-types a[rel^='all']").parent().addClass('active');
